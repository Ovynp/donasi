                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="metric">
                                ...
                                <p>
                                    <span class="number">{{$donatur->count()}}</span>
                        ...
                        <div class="col-md-3">
                            <div class="metric">
                                ...
                                <p>
                                    <span class="number">{{currency_IDR($kegiatan->target_donasi)}}</span>
                                    <span class="title">Target Donasi</span>
                                </p>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="metric">
                                ...
                                <p>
                                    <span class="number">{{currency_IDR($kegiatan->totalDonasi())}}</span>
                                    <span class="title">Donasi Terkumpul</span>
                                </p>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="metric">
                                ...
                                <p>
                                    <span class="number">{{Carbon\Carbon::parse($sekarang)->diffinDays($kegiatan->batas_donasi)}} hari</span>
									@if($kegiatan->batas_donasi >= $sekarang)
                                    <span class="title">Sisa Waktu Donasi</span>
									@else
									<span class="title">Lewat Waktu Donasi</span>
									@endif
                                </p>
                            </div>
                        </div>
					...
					<!-- PROFILE DETAIL -->
						<div class="profile-detail">
							<div class="profile-info">
                                <div class="text-center"><a href="/kegiatan/{{$user->id}}/{{$kegiatan->id}}/edit" class="btn btn-primary">Edit Profil</a></div>
							</div>
						</div>
					<!-- END PROFILE DETAIL -->
					</div>
                                    ...
                                    <div class="panel-body">
										<table class="table">
											...
											<tr>
												...
												<td><a href="/donatur/{{$kegiatan->id}}/file">{{$kegiatan->file_pendukung}}</a></td>
											</tr>
											<tr>
												<th>Batas Donasi</th>
								...
								<table class="table project-table">
                                    ...
                                    @foreach($kegiatan->panitias as $panitia)
										<tr>
											...
											<td>
												<a href="" class="lnr lnr-pencil" data-toggle="modal" data-target="#exampleModal1-{{$panitia->id}}"></a>
												<a href="/panitia/{{$panitia->id}}/delete" class="lnr lnr-trash" onclick="return confirm('Hapus Panitia ?')"></a>
											</td>
										</tr>
                                    @endforeach
									</tbody>
...
@foreach($kegiatan->panitias as $data)
<!-- EXAMPLE MODAL 1 -->
<div class="modal fade" id="exampleModal1-{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
        	...
        	<div class="modal-body">
                <form action="/panitia/{{$data->id}}/update" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
					...
					<div class="form-group {{$errors->has('no_telp') ? 'has-error' : ''}}">
						...
						<input name="no_telp" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{$data->no_telp}}">
        ...
        <div class="modal-content">
        	...
        	<div class="modal-body">
				...
				<div class="form-group {{$errors->has('nama') ? 'has-error' : ''}}">
						...
						<input name="kegiatan_id" type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nama Panitia" value="{{$panitia->kegiatan_id}}">
						@if($errors->has('nama') ? 'has-error' : '')
							<span class="help-block">{{$errors->first('nama')}}</span>
						@endif
					</div>
